  ...
  setKey: functions.https.onRequest((request, response) => {
    ...
    dbRef
      ...
      .then(() => {
        ...
        response.send("サーバーに鍵を用意した /get_public_keyで取得して");
      });
  }),
  getPublicKey: functions.https.onRequest((request, response) => {
    dbRef.get().then((doc) => {
      const vapidKeys = doc.data();
      console.log("Firebaseサーバー → ブラウザ:公開キー", vapidKeys.publicKey);
      console.log("Firebaseサーバー → ブラウザ:秘密キー", vapidKeys.privateKey);
  ...
  sendNotification: functions.https.onRequest((request, response) => {
    dbRef.get().then(async (doc) => {
      ...
      await webpush.sendNotification(
        ...
        {
          vapidDetails: {
            subject: "https://notification-sample-655a5.firebaseapp.com/",
            publicKey: vapidKeys.publicKey,
            privateKey: vapidKeys.privateKey,
          },
        }
